...
class URIDispatcher
{
	...
	private function match_request($http_method, $url_pattern, &$matched_parameters = array())
	{
		...
		if (
			...
			print_r("Request_METHOD= ".$_SERVER['REQUEST_METHOD']." vs http_method= ". strtoupper($http_method));
			if ($this->cors == false) {
				print("cors= " . $this->cors);
			}
		...
		if (sizeof($pathTokens) != sizeof($patternTokens)) {
			print("path= " . $path . " vs url_pattern= " . $url_pattern);
			print(" falso 2");
			return false;
		}
		...
		for ($i = 0; $i < sizeof($pathTokens); $i++) {
			if ($pathTokens[$i] != $patternTokens[$i]) {
				if (preg_match('/\$([0-9]+?)/', $patternTokens[$i], $matches) == 1) {
					...
					print("preg_match('/\$([0-9]+?)/', $patternTokens[$i], $matches)". preg_match('/\$([0-9]+?)/', $patternTokens[$i], $matches));
					print("falso 3");
					return false;
				}
			}
